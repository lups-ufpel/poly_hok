defmodule CudaBackend do
  ...
  defp gen_exp(exp) do
    case exp do
      ...
      # Esse pattern vai ser acionado quando estivermos acessando estruturas especiais de indexação da GPU (ex: threadIdx.x, blockIdx.y, etc.). É só retornar os átomos como estão. Caso seja alguma outra estrutura, o tratamentento é o mesmo.
      {{:., _, [{struct, _, _}, field]}, _, []} ->
        "#{to_string(struct)}.#{to_string(field)}"

      # Mesma ideia aqui. Não sei dizer quando que esse __aliases__ vai ser acionado, mas é bom deixarmos assim também.
